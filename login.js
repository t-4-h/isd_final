...
function validate(){
  ...
  for(var i = 0; i < objPeople.length; i++) {
    ...
    if(username == objPeople[i].username && password == objPeople[i].password) {
      ...
      window.location.href = "index.html";  // stop the statement if result is found true - this was a return in the video, break is best practice here
      break
    } else {
...
function registerUser() {
  ...
  for(var i = 0; i < objPeople.length; i++) {
    ...
    // check if new password is 8 characters or more
      ...
      alert('That is too short, include 4 or more characters')
      // stop the statement if result is found true
      break
    }
  }
