...
CREATE TABLE `hint` (
  `hintid` varchar(45) NOT NULL,
  `hint` varchar(500) DEFAULT NULL,
  `microid` varchar(45) DEFAULT NULL
...
CREATE TABLE `main` (
  ...
  `surgeryid` varchar(45) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `micro` (
  `microid` varchar(45) NOT NULL,
  `microstep` varchar(200) DEFAULT NULL,
...
--
ALTER TABLE `hint`
  ADD CONSTRAINT `microid` FOREIGN KEY (`microid`) REFERENCES `micro` (`microid`) ON DELETE SET NULL ON UPDATE CASCADE;

--
-- 限制表 `main`
--
ALTER TABLE `main`
  ADD CONSTRAINT `surgeryid` FOREIGN KEY (`surgeryid`) REFERENCES `procedure` (`surgeryid`) ON DELETE SET NULL ON UPDATE CASCADE;
